...
struct chipcounter_producer_subcommand {
   string producer_str;
   const name act_name{ N(chipcounter) };

   chipcounter_producer_subcommand(CLI::App* actionRoot) {
      auto chipcounter = actionRoot->add_subcommand("chipcounter", localized("Send producer chipcounter"));
      chipcounter->add_option("producer", producer_str, localized("producer"))->required();
      add_standard_transaction_options(chipcounter, "owner@active");
      chipcounter->callback([this] {
...
//                                ->check(CLI::ExistingFile);
   ...
   auto chipcounter = chipcounter_producer_subcommand(system);

...
class allowlist_denylist_tester {
   public:
      allowlist_denylist_tester() {}

      void init( bool bootstrap = true ) {
         FC_ASSERT( !chain, "chain is already up" );

         chain.emplace(tempdir, [&](controller::config& cfg) {
            cfg.sender_bypass_allowdenylist = sender_bypass_allowdenylist;
            cfg.actor_allowlist = actor_allowlist;
            cfg.actor_denylist = actor_denylist;
            cfg.contract_allowlist = contract_allowlist;
            cfg.contract_denylist = contract_denylist;
            cfg.action_denylist = action_denylist;
         }, !shutdown_called);
...
BOOST_AUTO_TEST_SUITE(allowlist_denylist_tests)

BOOST_AUTO_TEST_CASE( actor_allowlist ) { try {
   allowlist_denylist_tester<> test;
...
BOOST_AUTO_TEST_CASE( actor_denylist ) { try {
   allowlist_denylist_tester<> test;
   test.actor_denylist = {N(bob)};
   test.init();
   ...
   BOOST_CHECK_EXCEPTION( test.transfer( N(bob), N(alice) ),
                          actor_denylist_exception,
                          fc_exception_message_starts_with("authorizing actor(s) in transaction are on the actor denylist: [\"bob\"]")
                        );
...
BOOST_AUTO_TEST_CASE( contract_denylist ) { try {
   allowlist_denylist_tester<> test;
   test.contract_denylist = {N(charlie)};
   test.init();
   ...
   BOOST_CHECK_EXCEPTION( test.transfer( N(alice), N(charlie) ),
                          ...
                          fc_exception_message_is("account 'charlie' is on the contract denylist")
                        );
   ...
   BOOST_CHECK_EXCEPTION( test.chain->push_action( N(charlie), N(create), N(charlie), mvo()
                          ...
                          contract_denylist_exception,
                          fc_exception_message_starts_with("account 'charlie' is on the contract denylist")
                        );
   test.chain->produce_blocks();
} FC_LOG_AND_RETHROW() }

BOOST_AUTO_TEST_CASE( action_denylist ) { try {
   allowlist_denylist_tester<> test;
   test.contract_allowlist = {config::system_account_name, N(eosio.token), N(bob), N(charlie)};
   test.action_denylist = {{N(charlie), N(create)}};
   test.init();
   ...
   BOOST_CHECK_EXCEPTION( test.chain->push_action( N(charlie), N(create), N(charlie), mvo()
                          ...
                          fc_exception_message_starts_with("action 'charlie::create' is on the action denylist")
                        );
   test.chain->produce_blocks();
} FC_LOG_AND_RETHROW() }

BOOST_AUTO_TEST_CASE( denylist_eosio ) { try {
   allowlist_denylist_tester<tester> tester1;
   tester1.init();
   tester1.chain->produce_blocks();
   tester1.chain->set_code(config::system_account_name, contracts::eosio_token_wasm() );
   tester1.chain->produce_blocks();
   tester1.shutdown();
   tester1.contract_denylist = {config::system_account_name};
   tester1.init(false);

   allowlist_denylist_tester<tester> tester2;
   tester2.init(false);
...
BOOST_AUTO_TEST_CASE( deferred_denylist_failure ) { try {
   allowlist_denylist_tester<tester> tester1;
   tester1.init();
   tester1.chain->produce_blocks();
   ...
   tester1.contract_denylist = {N(charlie)};
   tester1.init(false);
...
BOOST_AUTO_TEST_CASE( denylist_onerror ) { try {
   allowlist_denylist_tester<TESTER> tester1;
   tester1.init();
   tester1.chain->produce_blocks();
   tester1.chain->set_code( N(bob), contracts::deferred_test_wasm() );
   ...
   tester1.action_denylist = {{config::system_account_name, N(onerror)}};
   tester1.init(false);
   ...
   BOOST_CHECK_EXCEPTION( tester1.chain->produce_blocks(), fc::exception,
                          fc_exception_message_is("action 'eosio::onerror' is on the action denylist")
                        );

} FC_LOG_AND_RETHROW() }

BOOST_AUTO_TEST_CASE( actor_denylist_inline_deferred ) { try {
   allowlist_denylist_tester<tester> tester1;
   tester1.init();
   tester1.chain->produce_blocks();
   ...
   // Disallow inline actions authorized by actor in denylist
   BOOST_CHECK_EXCEPTION( tester1.chain->push_action( N(alice), N(inlinecall), N(alice), mvo()
                                                         ( "contract", "alice" )
                                                         ( "authorizer", "bob" )
                                                         ( "payload", 10 ) ),
                           fc::exception,
                           fc_exception_message_is("authorizing actor(s) in transaction are on the actor denylist: [\"bob\"]") );
   ...
   tester1.actor_denylist = {N(bob), N(charlie)};
   tester1.init(false);
   ...
   // With charlie now in the actor denylist, retiring the previously scheduled deferred transaction should now not be possible.
   BOOST_CHECK_EXCEPTION( tester1.chain->produce_blocks(), fc::exception,
                          fc_exception_message_is("authorizing actor(s) in transaction are on the actor denylist: [\"charlie\"]")
                        );


   // With charlie now in the actor denylist, it is now not possible to schedule a deferred transaction authorized by charlie@active
...
BOOST_AUTO_TEST_CASE( denylist_sender_bypass ) { try {
   allowlist_denylist_tester<tester> tester1;
   tester1.init();
   tester1.chain->produce_blocks();
   tester1.chain->set_code( N(alice), contracts::deferred_test_wasm() );
   ...
   tester1.sender_bypass_allowdenylist = {N(charlie)};
   tester1.actor_denylist = {N(bob), N(charlie)};
   tester1.init(false);
   ...
   // Retire the deferred transaction successfully despite charlie being on the actor denylist.
   ...
   tester1.sender_bypass_allowdenylist = {N(charlie)};
   tester1.actor_denylist = {N(bob), N(charlie)};
   tester1.contract_denylist = {N(bob)}; // Add bob to the contract denylist as well
   tester1.init(false);

   num_deferred = tester1.chain->control->db().get_index<generated_transaction_multi_index,by_trx_id>().size();
   BOOST_REQUIRE_EQUAL(1, num_deferred);

   // Now retire the deferred transaction successfully despite charlie being on both the actor denylist and bob being on the contract denylist
   // This is allowed due to the fact that the sender of the deferred transaction (also charlie) is in the sender bypass list.
   tester1.chain->produce_blocks();
   ...
   // Ensure that if there if the sender is not on the sender bypass list, then the contract denylist is enforced.
   BOOST_CHECK_EXCEPTION( tester1.chain->produce_blocks(), fc::exception,
                          fc_exception_message_is("account 'bob' is on the contract denylist") );

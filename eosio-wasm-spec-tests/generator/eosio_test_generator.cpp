...
const set<string> denylist_memory_clearing = { "address.0",      "address.2",      "address.3",      "address.4",
...
const set<string> allowlist_force_check_throw = { "memory.6", "memory.7" };
...
void write_file(ofstream& file, string test_name, string funcs, string sub_applies, string apply) {
   ...
   out << sub_applies;
   out << apply_func;
   if (!denylist_memory_clearing.count(test_name)) {
      out << mem_clear;
   }
   out << apply;
   out << "   }\n";
...
int main(int argc, char** argv) {
   ...
   for (const auto& f : file_func_mappings) {
      ...
      for (auto test : f.second) {
         ...
         int is_allowlisted = allowlist_force_check_throw.count(test_name);
         if (type_test == "assert_trap" || type_test == "assert_exhaustion" || is_allowlisted) {

...
case $NAME in
    "Ubuntu")
       ( [[ "$(echo ${VERSION_ID})" == "18.04" ]] || [[ "$(echo ${VERSION_ID})" == "20.04" ]] ) || ( echo " - You must be running Ubuntu 18.04.x or 20.04.x to install EOSIO." && exit 1 )
    ;;
    "Linux Mint")
       ( [[ "$(echo ${VERSION_ID})" == "19" ]] || [[ "$(echo ${VERSION_ID})" == "20" ]] ) || ( echo " - You must be running Linux Mint 19 or 20 to install EOSIO." && exit 1 )
    ;;
...
if [[ $PIN_COMPILER == false ]]; then
  EXTRA_DEPS=(clang-10,dpkg\ -s llvm-10-dev,dpkg\ -s)

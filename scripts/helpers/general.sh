...
function install-package() {
  if [[ $ARCH == "Linux" ]]; then
    ...
    ( [[ $NAME == "CentOS Linux" ]] ) && eval $EXECUTION_FUNCTION $NEW_SUDO_COMMAND $YUM $OPTIONS install -y $1
    ( [[ $NAME =~ "Ubuntu" ]] || [[ $NAME =~ "Linux Mint" ]] ) && eval $EXECUTION_FUNCTION $NEW_SUDO_COMMAND $APTGET $OPTIONS install -y $1
  fi
...
function uninstall-package() {
  if [[ $ARCH == "Linux" ]]; then
    ...
    ( ( [[ $NAME == "CentOS Linux" ]] ) && [[ ! -z $(rpm -qa $1) ]] ) && eval $EXECUTION_FUNCTION $NEW_SUDO_COMMAND $YUM $OPTIONS $REMOVE -y $1
    ( ( [[ $NAME == "Linux Mint" ]] || [[ $NAME =~ "Ubuntu" ]] ) && $(dpkg -s $1 &>/dev/null) ) && eval $EXECUTION_FUNCTION $NEW_SUDO_COMMAND $APTGET $OPTIONS $REMOVE -y $1
  fi
  true
}

...
if [[ -z "${NAME}" ]]; then
    if [[ $ARCH == "Linux" ]]; then
        ...
        export NAME=$( cat /etc/os-release | grep ^NAME | cut -d'=' -f2 | sed 's/\"//gI' )
        export VERSION_ID=$( cat /etc/os-release | grep VERSION_ID | cut -d'=' -f2 | sed 's/[^0-9\.]//gI' )
...
elif [[ $NAME == "Ubuntu" ]] || [[ $NAME == "Linux Mint" ]]; then
...
function ensure-compiler() {
    ...
    if [[ $NAME == "CentOS Linux" ]] || ( $PIN_COMPILER ); then
        export CXX=${CXX:-'g++'}
        export CC=${CC:-'gcc'}
    fi

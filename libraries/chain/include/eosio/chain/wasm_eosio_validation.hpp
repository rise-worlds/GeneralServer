...
namespace eosio { namespace chain { namespace wasm_validations {
   ...
   struct denylist_validator {
      static constexpr bool kills = true;
      static constexpr bool post = false;
      static void accept( wasm_ops::instr* inst, wasm_ops::visitor_arg& arg ) {
         FC_THROW_EXCEPTION(wasm_execution_error, "Error, denylisted opcode ${op} ",
            ("op", inst->to_string()));
      }
   };
   ...
   struct op_constrainers : wasm_ops::op_types<denylist_validator> {

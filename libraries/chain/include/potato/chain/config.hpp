...
namespace potato::chain::config
{
    ...
    // const static potato::chain::wasm_interface::vm_type default_wasm_runtime = potato::chain::wasm_interface::vm_type::wabt;
...
{
    return (value * percentage) / potato::chain::config::percent_100;
}

template <typename Number>
Number EOS_PERCENT_CEIL(Number value, uint32_t percentage)
{
    return ((value * percentage) + potato::chain::config::percent_100 - potato::chain::config::percent_1) / potato::chain::config::percent_100;
}

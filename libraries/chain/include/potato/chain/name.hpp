...
namespace fc {
    class variant;
    void to_variant(const potato::chain::name& c, fc::variant& v);
    void from_variant(const fc::variant& v, potato::chain::name& check);
} // fc
...
namespace potato::chain
{
    ...
    private:
        ...
        friend void fc::from_variant(const fc::variant &v, potato::chain::name &check);
...
#define N(X) potato::chain::string_to_name(#X)
} // namespace potato::chain

namespace std {
   template<> struct hash<potato::chain::name> : private hash<uint64_t> {
      typedef potato::chain::name argument_type;
...
FC_REFLECT( potato::chain::name, (value) )
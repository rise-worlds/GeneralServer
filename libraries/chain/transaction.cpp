...
#include <potato/chain/action.hpp>
#include <potato/chain/transaction.hpp>
...
namespace potato::chain
{
   ...
   packed_transaction::packed_transaction(bytes &&packed_txn, vector<signature_type> &&sigs, bytes &&packed_cfd, compression_type _compression)
       : signatures(std::move(sigs)), compression(_compression), packed_context_free_data(std::move(packed_cfd)), packed_trx(std::move(packed_txn))
   {
   ...
   packed_transaction::packed_transaction(transaction &&t, vector<signature_type> &&sigs, bytes &&packed_cfd, compression_type _compression)
       : signatures(std::move(sigs)), compression(_compression), packed_context_free_data(std::move(packed_cfd)), unpacked_trx(std::move(t), signatures, {}), trx_id(unpacked_trx.id())
   {
